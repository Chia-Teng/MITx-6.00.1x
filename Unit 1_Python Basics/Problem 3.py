...
# check each potential start point from 1st through -2nd char
    ...
    for rest in range(start+1,len(s)): # check each char following start point
        ...
        for abc in alphabet:
        # match each alphebet to current then previous char

            if abc == s[rest-1]:
                t_answer += s[rest]
                # if the previous char matches first, extend t_answer
                break
                # then stop matching rest of alphabets
            
            elif abc == s[rest]:
                break
            # if current char matches first,  stop matching rest of alphabets
            
        if abc != s[rest-1]:
            break
        # if t_answer doesn't extend, stop checking rest of s

    if len(t_answer) > len(answer):
        answer = t_answer
    # only keep the first longest result
